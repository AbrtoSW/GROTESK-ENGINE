...
std::filesystem::file_time_type ShaderUtil::getFileTimeStamp(const std::string& shaderFile) {
	auto full = Asset(shaderFile);
	return std::filesystem::last_write_time(full);
}
...
glslang::TShader::Includer::IncludeResult* RuntimeIncluder::includeLocal(const char* headerName, const char* includerName, size_t includeDepth) {
	...
	auto full = Asset(std::filesystem::path("shaders") / headerName);
...
VkShaderModule ShaderUtil::compileToSPV(VkDevice device, const std::string& shaderFile, EShLanguage stage) {
	...
	auto ends_with_spv = [](const std::string& s) {
	...
	auto full = Asset(shaderFile);          // resolve once
	auto fullStr = full.string();

	if (ends_with_spv(shaderFile)) {
		// read binary file
		std::vector<char> bytes;
		{
			std::ifstream f(shaderFile, std::ios::binary | std::ios::ate);
			if (!f) {
				throw std::runtime_error("Failed to open SPIR-V file: " + shaderFile);
			}
			std::streamsize sz = f.tellg();
			if (sz <= 0) {
				throw std::runtime_error("Empty SPIR-V file: " + shaderFile);
			}
			bytes.resize(static_cast<size_t>(sz));
			f.seekg(0, std::ios::beg);
			if (!f.read(bytes.data(), sz)) {
				throw std::runtime_error("Failed to read SPIR-V file: " + shaderFile);
			}
		}

		if ((bytes.size() % 4) != 0) {
			throw std::runtime_error("SPIR-V size not multiple of 4: " + shaderFile);
		}
		...
		if (vkCreateShaderModule(device, &ci, nullptr, &mod) != VK_SUCCESS) {
			throw std::runtime_error("Failed to create shader module from SPIR-V: " + shaderFile);
		}
		return mod;
	}

	// else: treat as GLSL source and compile at runtime (your existing path)
	std::string source = readFile(fullStr);
